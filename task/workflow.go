...
// Workflow is the functional interface for orchestrator functions.
type Workflow func(ctx *WorkflowContext) (any, error)

// WorkflowContext is the parameter type for orchestrator functions.
...
// callChildWorkflowOptions is a struct that holds the options for the CallChildWorkflow orchestrator method.
...
// ChildWorkflowOption is a functional option type for the CallChildWorkflow orchestrator method.
...
// WithChildWorkflowAppID is a functional option type for the CallChildWorkflow orchestrator method that specifies the app ID of the target activity.
...
func NewWorkflowContext(registry *TaskRegistry, id string, oldEvents []*protos.HistoryEvent, newEvents []*protos.HistoryEvent) *WorkflowContext {
...
func (ctx *WorkflowContext) processEvent(e *protos.HistoryEvent) error {
	...
	if os := e.GetWorkflowStarted(); os != nil {
		// WorkflowStarted is only used to update the current orchestration time and history patches
		...
		err = ctx.onTaskScheduled(e.GetEventID(), ts)
		...
		err = ctx.onChildWorkflowScheduled(e.GetEventID(), ts)
...
func (ctx *WorkflowContext) CallChildWorkflow(orchestrator interface{}, opts ...ChildWorkflowOption) Task {
	...
	if options.retryPolicy != nil {
		return ctx.internalScheduleTaskWithRetries(orchestratorName+"-retry", ctx.CurrentTimeUtc, func(_ string) Task {
			return ctx.internalCallChildWorkflow(orchestratorName, options)
		}, *options.retryPolicy, 0, uuid.NewString())
	}
...
func (ctx *WorkflowContext) internalCallChildWorkflow(orchestratorName string, options *callChildWorkflowOptions) Task {
...
func (ctx *WorkflowContext) onExecutionStarted(es *protos.ExecutionStartedEvent) error {
	orchestrator, err := ctx.getWorkflow(es)
	if err != nil {
		return err
	}
	ctx.Name = es.Name
...
func (ctx *WorkflowContext) onChildWorkflowScheduled(taskID int32, ts *protos.ChildWorkflowInstanceCreatedEvent) error {
	if a, ok := ctx.pendingActions[taskID]; !ok || a.GetCreateChildWorkflow() == nil {
		return fmt.Errorf(
			"a previous execution called CallChildWorkflow for '%s' and sequence number %d at this point in the orchestration logic, but the current execution doesn't have this action with this sequence number",
			ts.Name,
			taskID,
		)
	}
...
func (ctx *WorkflowContext) onTimerCreated(e *protos.HistoryEvent) error {
	if a, ok := ctx.pendingActions[e.GetEventID()]; !ok || a.GetCreateTimer() == nil {
		return fmt.Errorf(
	...
	delete(ctx.pendingActions, e.GetEventID())
	return nil
}
...
func (ctx *WorkflowContext) onExecutionTerminated(et *protos.ExecutionTerminatedEvent) error {
	if err := ctx.setCompleteInternal(et.Input, protos.WorkflowStatus_WORKFLOW_STATUS_TERMINATED, nil); err != nil {
		return err
	}
	return nil
}
...
func (ctx *WorkflowContext) setFailed(appError error) error {
	...
	failedStatus := protos.WorkflowStatus_WORKFLOW_STATUS_FAILED
...
func (ctx *WorkflowContext) setContinuedAsNew() error {
	status := protos.WorkflowStatus_WORKFLOW_STATUS_CONTINUED_AS_NEW
